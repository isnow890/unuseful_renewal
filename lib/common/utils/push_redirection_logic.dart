...
import '../../hit_schedule/provider/hit_schedule_provider.dart';
import '../../hit_schedule/provider/hit_schedule_selected_day_provider.dart';
import '../../hit_schedule/view/hit_schedule_main_screen.dart';
import '../provider/drawer_selector_provider.dart';
import '../provider/fcm_router_provider.dart';
...
routerLogicForegroundHitDutyAlarmWidgetRef1(WidgetRef ref) {
  ...
  if (fromAlarm.page == 'hitSchedule1') {
    if (fromAlarm.action == 'selectDay') {
      ref.read(hitScheduleSelectedDayProvider.notifier).update((state) =>
          DateTime(
            int.parse(fromAlarm.param.substring(0, 4)),
            int.parse(fromAlarm.param.substring(5, 7)),
            int.parse(fromAlarm.param.substring(8, 10),),),);
      ref
